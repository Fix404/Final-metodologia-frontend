...
import { FaUser, FaMapMarkerAlt, FaCreditCard, FaShieldAlt } from "react-icons/fa";
import { 
  setDni, 
  setDireccionEnvio, 
  setMetodoPago, 
...
const usuario: IUsuario = {
  ...
  const [direccionTemporal, setDireccionTemporal] = useState({
    ...
    numero: "",
    codigoPostal: "",
    localidad: { id: 0, nombre: "", provincia: "" }
  });
  ...
  useEffect(() => {
    ...
    /* COMENTADO PARA TESTING - Descomentar cuando tengas auth real
    ...
    */
    ...
    // Verificar si necesita DNI
    setMostrarFormularioDni(!usuario.dni && !compraState.dni);
    
    // Verificar si necesita dirección
    setMostrarFormularioDireccion(!usuario.direccion && !compraState.direccionEnvio);
  ...
  const handleGuardarDni = () => {
    if (dniTemporal.length >= 7) {
      dispatch(setDni(dniTemporal));
      setMostrarFormularioDni(false);
    }
  };

  const handleGuardarDireccion = () => {
    if (direccionTemporal.calle && direccionTemporal.numero) {
  ...
  const handleSeleccionarMetodoPago = (metodo: 'transferencia' | 'mercadopago') => {
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center mb-4">
                ...
                <h2 className="text-xl font-semibold text-gray-800">Datos del comprador</h2>
              </div>
              
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <p className="text-gray-700"><strong>Nombre:</strong> {usuario.nombre} {usuario.apellido}</p>
                  <p className="text-gray-700"><strong>Email:</strong> {usuario.email}</p>
                </div>
                ...
                <div>
                  {mostrarFormularioDni ? (
                    ...
                    <p className="text-gray-700">
                      <strong>DNI:</strong> {compraState.dni || usuario.dni || 'No especificado'}
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="space-y-4">
                  {usuario.direccion && (
                    <div className="bg-gray-50 p-4 rounded-md">
                      <p className="text-sm text-gray-600 mb-2">¿Usar tu dirección guardada?</p>
                      <p className="text-gray-700">
                        {usuario.direccion?.calle} {usuario.direccion?.numero}, {usuario.direccion?.localidad?.nombre}
                      </p>
                      <button
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    <input
                      ...
                      onChange={(e) => setDireccionTemporal({...direccionTemporal, calle: e.target.value})}
                    ...
                    <input
                      ...
                      onChange={(e) => setDireccionTemporal({...direccionTemporal, numero: e.target.value})}
                      className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4A90E2]"
                    />
                    <input
                      type="text"
                      ...
                      onChange={(e) => setDireccionTemporal({...direccionTemporal, codigoPostal: e.target.value})}
                      className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4A90E2]"
                    />
                    <input
                      type="text"
                      ...
                      onChange={(e) => setDireccionTemporal({
                        ...direccionTemporal, 
                        localidad: {...direccionTemporal.localidad, nombre: e.target.value}
                      })}
                      className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#4A90E2]"
                    />
                  </div>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    {compraState.direccionEnvio ? (
                      <p className="text-gray-700">
                        {compraState.direccionEnvio.calle} {compraState.direccionEnvio.numero}<br/>
                        {compraState.direccionEnvio.localidad.nombre}, {compraState.direccionEnvio.localidad.provincia}
                        {compraState.direccionEnvio.codigoPostal && ` - CP: ${compraState.direccionEnvio.codigoPostal}`}
                      </p>
                    ) : usuario.direccion ? (
                      <p className="text-gray-700">
                        {usuario.direccion?.calle} {usuario.direccion?.numero}<br/>
                        {usuario.direccion?.localidad.nombre}, {usuario.direccion?.localidad.provincia}
              ...
              <div className="space-y-3">
                <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                  <input
                    ...
                    onChange={() => handleSeleccionarMetodoPago('transferencia')}
                    className="mr-3"
                  />
                  <div>
                    <p className="font-medium text-gray-800">Transferencia Bancaria</p>
                    <p className="text-sm text-gray-600">Te enviaremos los datos para realizar la transferencia</p>
                  </div>
                </label>
                ...
                <label className="flex items-center p-4 border rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">Paga con tarjeta, efectivo o dinero en cuenta</p>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3 mb-4">
                {items.map(({ detalle, cantidad }) => (
                  <CompraCard key={detalle.id} detalle={detalle} cantidad={cantidad} />
                ))}
              </div>
